...
/*9. Разработать набор классов (минимум 5 классов, связи между классами: агрегация, композиция, наследование) по предметной области
«Магазин стройматериалов». Функционал программы должен позволить собрать заказ. Сгенерировать минимум пять типов
...
void addProd(BuildMaterials& build_mat, Order& cur_order) {
	...
	int cursor = intInput("порядковый номер товара из списка выше", 0, build_mat.getSize());
	if (cursor-- == 0)
		throw ex;
...
void completeOrder(Order& ord) {
	...
	to_write += "Покупатель: ";
	to_write += buyer.getName() += " - ";
	to_write += buyer.getFace() ? "Физ. лицо\n" : "Юр. лицо\n";
	to_write += "Продавец: ";
	to_write += empl.getName() += " - ";
	to_write += empl.getInitials() += "\n-=-Успешно-оплачено-=-\n";
...
int main() {
	...
	try {
		BuildMaterials build_m(NULL);
		bool state = true;
		while (state) {
			cout << "\nВыберите действие нажатием клавиши:\n"
				<< "\n\t1 - Собрать заказ\n"
				<< "\tESC - Выйти\n";
			int input = _getch();
			switch (input) {
	...
	set_terminate(myOwnExit);
	throw "Подачка-неберучка"; //компилятор не поймёт
	return 1;
}